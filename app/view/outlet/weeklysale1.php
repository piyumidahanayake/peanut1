      ...
      <tr> 
       <td>
        <div class="btn-hlock" style="align:left; padding-bottom:13px; padding-right:10px; padding-left:10px;">
                <button type="select" href="/" disabled style="background: green">Selected By</button> </div><td padding-width="1200px"></td>
...
<form action="<?php echo URLROOT; ?>/Outletpages/weeklysale1" method="post">
  ...
  <?php 
  $t=0;
  foreach($data['result1'] as $result1) : ?>

    <?php 
    $q=0;
    $p=0;
    
    foreach($data['result'] as $result2) : 
          if($result1->id==$result2->id){
              $q=$q+$result2->quantity;
              $p=$p+$result2->money_receive;
          }
